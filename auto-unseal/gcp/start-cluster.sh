...
if ! docker-compose -p gcp up -d; then
...
if [[ "$Dont_Init_Flag" = "ni" ]]; then
    ...
    docker-compose -p gcp down
    echo -e ${Yellow}Please try again.${Color_Off}
    exit 0
else
    echo -e ${Yellow}Initializing gcp cluster${Color_Off}
    sleep 2
    VAULT_ADDR=http://127.0.0.1:8222 vault operator init -format=json -recovery-shares=1 -recovery-threshold=1 | jq -r > ./init.json
    echo -e ${Yellow}Logging in with root token.${Color_Off}
    VAULT_ADDR=http://127.0.0.1:8222 vault login $(cat ./init.json| jq -r '.root_token')
fi
fi
fi